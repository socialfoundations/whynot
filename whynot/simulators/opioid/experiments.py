...
def sample_initial_states(rng):
    ...
    return opioid.State(nonmedical_users=nonmedical_users, oud_users=oud_users,
                        illicit_users=illicit_users)


@parameter(name="nonmedical_incidence_delta",
           default=-0.113, values=[-0.073, -0.113],
...
def opioid_intervention(nonmedical_incidence_delta):
    ...
    return opioid.Intervention(time=2015, nonmedical_incidence=nonmedical_incidence_delta)
...
########################
@parameter(name="propensity", default=0.9, values=[0.5, 0.6, 0.7, 0.9, 0.99],
...
def confounded_propensity_scores(config, intervention, control_runs, propensity):
    ...
    illicit_deaths = [opioid.simulator.compute_illicit_deaths(run, 2015, config, intervention)
    ...
    propensity_scores = (1. - propensity) * np.ones(num_samples)
...
UnobservedConfounding = DynamicsExperiment(
    ...
    description=("Opioid unobserved confounding by treating runs with high "
    ...
    propensity_scorer=confounded_propensity_scores,
    outcome_extractor=overdose_deaths,
    covariate_builder=lambda intervention, run: np.array([run[intervention.time].oud_users,
                                                          run[intervention.time].nonmedical_users]))


########################
# Mediation experiments
########################
@parameter(name="mediation_year", default=2020, values=[2017, 2019, 2020, 2021, 2024],
