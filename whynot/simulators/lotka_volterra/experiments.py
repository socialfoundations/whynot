...
@parameter(name="window", default=10, values=[5, 10, 20],
           description="Number of years to inspect outcome.")
@parameter(name="outcome_year", default=80, values=[30, 50, 80],
...
def lv_outcome_extractor(run, window, outcome_year):
    ...
    return np.min([run[year].foxes for year in range(outcome_year - window, outcome_year)])


###########################
# RCT Experiment
###########################
@parameter(name="propensity", default=0.1, values=np.linspace(0.05, 0.5, 10),
...
##########################
@parameter(name="propensity", default=0.9, values=np.linspace(0.5, 0.99, 10),
