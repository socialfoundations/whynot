...
paper_rc = {'lines.linewidth': 3.0, 'lines.markersize': 4, 'lines.markeredgewidth': 2, 'errorbar.capsize': 2,
            'figure.figsize': (10, 6), 'xtick.labelsize': 18, 'ytick.labelsize': 18,
        ...
        for i, illicit_exit in enumerate(illicit_exits):
            for j, delta in enumerate(nonmedical_incidence_deltas):
                ...
                experiment = DynamicsExperiment(
                    name="opioid_rct",
                    description=("Randomized experiment reducing nonmedical incidence of "
                                 "opioid use in 2015."),
                    simulator=opioid,
...
def effects_histogram(true_effects, estimated_effects=None, num_bins=100,
                      title=None, x_range=None, y_max=None, show_avg=True):
    ...
    if estimated_effects is not None and x_range is None:
        x_range = np.percentile(
            np.concatenate((true_effects, estimated_effects)),
            (5, 95)
        )

    true_effects_mean = np.mean(true_effects)
    plt.axvline(x=true_effects_mean, color='C0', linestyle='--')
    plt.hist(true_effects, bins=num_bins, range=x_range, density=True, alpha=0.5,
             color='C0', label='True Effects')
    if estimated_effects is not None:
        ...
        plt.hist(estimated_effects, bins=num_bins, range=x_range, density=True, alpha=0.5,
                 color='C1', label='Estimated Effects')

    if y_max:
        plt.ylim(top=y_max * 1.05)

    if show_avg:
        plt.text(true_effects_mean, plt.ylim()[1] * 0.95, '  True ATE',
                 color='C0', fontsize=18)
        if estimated_effects is not None:
            plt.text(estimated_effects_mean, plt.ylim()[1] * 0.9, '  Estimated ATE',
...
def effects_histogram_slider(data_dir='heterogeneous_example_data', show_estimated_effects=False):
    if data_dir[-1] != '/':
        data_dir += '/'
    data_filename = data_dir + 'heterogeneous_example_data.json'
    data = json.load(open(data_filename))
        ...
        for illicit_exit in data[n].keys():
            for delta in data[n][illicit_exit].keys():
                ...
                if show_estimated_effects:
                    x_min = min(x_min, np.percentile(one_run_data['estimated_effects'], 10))
                    x_max = max(x_max, np.percentile(one_run_data['estimated_effects'], 90))
    ...
    for n in data.keys():
        for illicit_exit in data[n].keys():
            for delta in data[n][illicit_exit].keys():
                one_run_data = data[n][illicit_exit][delta]
                weights, _ = np.histogram(one_run_data['true_effects'], bins=num_bins,
                                          range=x_range, density=True)
                y_max = max(y_max, max(weights))
                if show_estimated_effects:
                    weights, _ = np.histogram(one_run_data['estimated_effects'], bins=num_bins,
                                              range=x_range, density=True)
                    y_max = max(y_max, max(weights))
    ...
    def plot_func(n, illicit_exit, delta):
        ...
        effects_histogram(one_run_data['true_effects'], estimated_effects, num_bins=num_bins,
                          title=title, x_range=x_range, y_max=y_max, show_avg=False)

    sample_sizes = sorted(data.keys(), key=lambda x: int(x))
    illicit_exits = sorted(data[sample_sizes[0]].keys(), key=lambda x: float(x))
    deltas = sorted(data[sample_sizes[0]][illicit_exits[0]].keys(), key=lambda x: float(x))
...
def plot_relative_error(data_dir='heterogeneous_example_data'):
    ...
    plt.figure(figsize=(18, 8))
    ax = plt.gca()
    grid = sns.lineplot(x='Sample Size', y='Relative Error',
                        data=plotting_df, marker="o", ax=ax)
